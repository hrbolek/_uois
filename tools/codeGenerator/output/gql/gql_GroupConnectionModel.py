...
class get_GroupConnectionModel(graphene.ObjectType):
...
class create_GroupConnectionModel(graphene.Mutation):
    ...
    ok = graphene.Boolean()
    result = graphene.Field('get_GroupConnectionModel')
    
    def mutate(parent, info, **paramList):
        session = extractSession(info)
        result = GroupConnectionModel(**paramList)
        session.add(result)
        session.commit()
...
class update_GroupConnectionModel(graphene.Mutation):
    ...
    def mutate(parent, info, **paramList):
        session = extractSession(info)
        dbRecord = session.query(GroupConnectionModel).filter_by(id=paramList['id']).one()
...
def resolve_groups_groups_by_id(root, info, id):
    session = extractSession(info)
    dbRecord = session.query(GroupConnectionModel).filter_by(id=id).one()
    return dbRecord
def resolve_groups_groups_child_id_starts_with(root, info, child_id):
    session = extractSession(info)
    dbRecords = session.query(GroupConnectionModel).filter(GroupConnectionModel.child_id.startswith(child_id)).all()
    return dbRecords
def resolve_groups_groups_parent_id_starts_with(root, info, parent_id):
    session = extractSession(info)
    dbRecords = session.query(GroupConnectionModel).filter(GroupConnectionModel.parent_id.startswith(parent_id)).all()
    return dbRecords