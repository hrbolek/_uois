...
class AcProgramGQLModel:
    ...
    async def language(self, info: strawberryA.types.Info) -> 'AcProgramLanguageTypeGQLModel':
        async with withInfo(info) as session:
...
@strawberryA.federation.type(keys=["id"], description="Program form type (Present, distant, ...)")
...
@strawberryA.federation.type(keys=["id"], description="Classification on the end of semesters")
...
class AcSubjectGQLModel:
    ...
    @strawberryA.field(description="""StudyProgramID""")
    async def semesters(self, info: strawberryA.types.Info) -> List['AcSemesterGQLModel']:
        async with withInfo(info) as session:
...
class AcSemesterGQLModel:
    ...
    async def classification(self, info: strawberryA.types.Info) -> 'ClassificationTypeGQLModel':
        async with withInfo(info) as session:
            result = await resolveClassificationTypeById(session, self.classificationtype_id)
            return result
...
class Query:
    ...
    async def say_hello_granting(self, info: strawberryA.types.Info, id: str) -> Union[str, None]:
        result = f'Hello {id} from granting'
        return result

    async def program_page(self, info: strawberryA.types.Info, skip: int=0, limit: int=10) -> List['AcProgramGQLModel']:
        async with withInfo(info) as session:
            result = await resolveProgramPage(session, skip=skip, limit=limit)
            return result

    async def program_for_group(self, info: strawberryA.types.Info, group_id: strawberryA.ID) -> List['AcProgramGQLModel']:
        async with withInfo(info) as session:
            result = await resolveProgramForGroup(session, id=group_id)
            return result

    async def program_json(self, info: strawberryA.types.Info, id: strawberryA.ID) -> Union['JSON', None]:
        async with withInfo(info) as session:
