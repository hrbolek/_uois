...
class MembershipGQLModel:
    ...
    async def startdate(self) -> datetime.datetime:
        return self.startdate

    @strawberryA.field(description="""date when the membership ends""")
    async def enddate(self) -> datetime.datetime:
        return self.enddate
...
class UserGQLModel:
    ...
    async def resolve_references(cls, info: strawberryA.types.Info, id: typing.List[strawberryA.ID]):
        ...
        async with withInfo(info) as session:
            ...
            for item in results:
                item._type_definition = cls._type_definition # little hack :)
            return results
...
class GroupGQLModel:
    ...
    async def resolve_references(cls, info: strawberryA.types.Info, id: typing.List[strawberryA.ID]):
        async with withInfo(info) as session:
            results = await resolveGroupsById(session, id)
            for item in results:
                if item is None:
                    continue
                item._type_definition = cls._type_definition # little hack :)           
    ...
    async def grouptype(self, info: strawberryA.types.Info) -> Union['GroupTypeGQLModel', None]:
        async with withInfo(info) as session:
...
class GroupEditorGQLModel:
    ...
    @strawberryA.field(description="""Description of operation (group update) result. "ok", if all went good.""")
    def result(self) -> str:
        return self.result
    ...
    async def update(self, info: strawberryA.types.Info, group: GroupUpdateGQLModel) -> 'GroupEditorGQLModel':
        async with withInfo(info) as session:
    ...
    async def create_user(self, info: strawberryA.types.Info, user: UserUpdateGQLModel) -> 'UserGQLModel':
        #session = AsyncSessionFromInfo(info)
        async with withInfo(info) as session:
            print('create_user')
