...
async def resolveExternalIdIntoInnerId(session, externalid, typeid):
    ...
    stmt = select(ExternalIdModel).filter(ExternalIdModel.typeid_id == typeid, ExternalIdModel.outer_id == externalid)
...
async def resolveInnerIdIntoExternalIds(session, internalid, typeid=None):
    ...
    if typeid is None:
        ...
        stmt = select(ExternalIdModel).filter(ExternalIdModel.typeid_id == typeid, ExternalIdModel.inner_id == internalid)
...
async def resolveAssignExternalId(session, internalid, externalid, typeid):
    ...
    if dbRecord is None:
        dbRecord = ExternalIdModel(typeid_id=typeid, inner_id=internalid, outer_id=externalid)
