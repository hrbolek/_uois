...
export const BuildingMedium = (props) => {
    return (
        <Card>
            <Card.Header>
                Budova {props.name} ({props.id}) <br /> 
                Areál {JSON.stringify(props.areal)} <br />
                Odpovědná osoba  {JSON.stringify(props.user)} <br />
            </Card.Header>
            <Card.Body>
...
/**
 * 
 * @param {*} response JSON structure which has been got from GQL API
 * @returns JSON structure which helds data about building
 */
const getDataFromJSON = (response) => response.data.building;
...
export const BuildingFetching = (props) => {
    ...
    const [state, error] = useQueryGQL(props.id, queryFunc, getDataFromJSON)
...
export const BuildingPage = (props) => {
    ...
    //pro demonstraci vizualizace bez stahovani dat z API GQL
    return <BuildingLargeStoryBook {...props} id={id} />;

    // pro vizualizaci se stahovanim dat z API GQL
