...
function getBackgroundColor(group) {
    let backgroundColor = 'rgb(230, 255, 230)';
    if (group?.grouptype) {
        backgroundColor = groupTypes[group?.grouptype.id]?.color;
    }
...
const GroupSmall = ({group}) => {
    return (
        <Link tag="group" id={group.id}><PeopleFill /> {group.name}</Link>
    )
}
...
const createMaster = (group) => {
    ...
    if (mastergroup) {
        ...
        return (props) => (
            <MasterComponent>
                <TreeNode label={<div style={{'backgroundColor': backgroundColor, 'border': '1px solid black', 'padding': '5px'}}><GroupSmall group={group} /></div>}>
                    {props.children}
                </TreeNode>
            </MasterComponent>
        )
        ...
        return (props) => (
            <Tree label={<div style={{'backgroundColor': backgroundColor, 'border': '1px solid black', 'padding': '5px'}}><GroupSmall group={group} /></div>}>
                {props.children}
            </Tree>
        )
    }
...
export const GroupSchema = ({group}) => {   
    ...
    return (
        ...
        // </Card>
        <>
            <BuildAcademicMasterNode group={{...group, subgroups: academicGroups}} />
            <hr />
        </>
    )
}
