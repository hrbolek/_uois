...
from gql_publications.GraphResolvers import resolvePublicationById,resolvePublicationAll, resolveAuthorById
...
from gql_publications.GraphResolvers import resolveUpdateAuthor, resolveInsertAuthor, resolveUpdateAuthorOrder
...
class PublicationEditorGQLModel:
    ...
    async def set_author_share(self, info: strawberryA.types.Info, author_id: uuid.UUID, share: float) -> 'AuthorGQLModel':
        async with withInfo(info) as session:
            result = await resolveUpdateAuthor(session, author_id, data=None, extraAttributes={'share': share})
            return result
